    ...
    <div class="flex justify-center pl-14 py-10 bg-stone-800">
        <div class="z-0 mb-8 bg-stone-700 p-2 border rounded-md">
            <div class="flex w-[950px] items-center border-b p-1">
                <h1 class="flex text-xl text-stone-100 font-bold tracking-wider w-[800px]">DATA SPK CETAK</h1>
                <div class="flex w-full justify-end items-center">
                    ...
                    @canany(['isAdmin', 'isMarketing'])
                        @can('isOrder')
                            @can('isMarketingEdit')
                                <a href="/marketing/print-orders/{{ $print_order->id }}/edit"
                    ...
                    @endcanany
                    @can('isAdmin')
                        <form action="/marketing/print-orders/{{ $print_order->id }}" method="post" class="d-inline my-1">
                            @method('delete')
                            @csrf
                            <button class="flex items-center justify-center btn-danger mx-1"
                                onclick="return confirm('Apakah anda yakin ingin menghapus data SPK cetak dengan nomor {{ $print_order->number }} ?')">
                                <svg class="fill-current w-5" clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round"
                                    stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path
